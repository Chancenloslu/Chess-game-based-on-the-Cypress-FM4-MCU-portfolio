...
void controlLeds() {
    ...
    // joystick middle
    else if (analog16 < 220 && analog16 >= 175) {
        FM4_GPIO->PDORB_f.P2 = 1u; // green off
        FM4_GPIO->PDOR1_f.P8 = 0u; // blue on
        FM4_GPIO->PDOR1_f.PA = 1u; // red off
    }
    // joystick right
    else if (analog16 < 175) {
        FM4_GPIO->PDORB_f.P2 = 1u; // green off
        FM4_GPIO->PDOR1_f.P8 = 1u; // blue off
...
void printValues() {
    ...
    setCursor(0, 319); // set to top-left corner
    char freeSpace[] = " ";
    ...
    writeText("  Joystick 1 X-Achse: ");
    writeNumberOnDisplayRight_s(&analog16);
    writeTextln("");
    writeText("  Joystick 1 Y-Achse: ");
    writeNumberOnDisplayRight_s(&analog19);
    writeTextln("");

    writeText("  Joystick 2 X-Achse: ");
    writeNumberOnDisplayRight_s(&analog13);
    writeTextln("");
    writeText("  Joystick 2 Y-Achse: ");
